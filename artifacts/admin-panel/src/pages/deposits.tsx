...
export default function Deposits() {
  ...
  const { data, isLoading } = useGetAdminDeposits(
    { page, limit: 20, status: status === "ALL" ? undefined : status },
    { query: { queryKey: getGetAdminDepositsQueryKey({ page, limit: 20, status: status === "ALL" ? undefined : status }) } }
  );
    ...
    approveMutation.mutate(
      { id, data: { status: action } },
      {
        onSuccess: () => {
          toast({
            title: "Success",
            description: `Deposit ${action.toLowerCase()} successfully`,
          });
          queryClient.invalidateQueries({ queryKey: getGetAdminDepositsQueryKey({ page, limit: 20, status: status === "ALL" ? undefined : status }) });
        },
        onError: (err) => {
          toast({
            title: "Error",
                  ...
                  <TableBody>
                    {data.deposits.map((tx) => (
                      <TableRow key={tx.id}>
                        ...
                        <TableCell className="font-mono text-xs">{tx.utr || tx.referenceId || '-'}</TableCell>
                        <TableCell>
              ...
              <div className="flex items-center justify-between pt-4">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button 
                    ...
                    disabled={data.deposits.length < 20}
                  >
                    Next
                  </Button>
                </div>
